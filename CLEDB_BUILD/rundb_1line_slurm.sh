...
dbconf=$(sed -n 3p $chomedir/config/DB.INPUT)                               ## read database parameter configuration
yh=$(echo $dbconf | sed -n 's/^[^0-9]*\([0-9]\{1,\}\).*$/\1/p')            ## requested number of Y heights
ln=$(sed -n 5p $chomedir/config/DB.INPUT)                                   ## parse parameter ranges line
...
if (($yh > $ncore)); then                                                                                 ## if generating more database heights than available threads
	...
	for e in `seq 0 $(echo "scale=0 ; $ncore - 1 " | bc)`; do                                              ## cycle through available threads
		mkdir $e;
		cd $e;
		\cp $chomedir/config/{DB.INPUT,GRID.DAT,IONEQ} ./;                                              ## make individual run dirs and copy necessary files
