...
homedir=$(pwd)                                ## The CLEDB_BUILD setup dir; script is located here in default config; change only if running script from different dir;
...
dbconf=$(sed -n 3p $homedir/config/DB.INPUT)                               ## read database parameter configuration
yh=$(echo $dbconf | sed -n 's/^[^0-9]*\([0-9]\{1,\}\).*$/\1/p')            ## requested number of Y heights
ln=$(sed -n 5p $homedir/config/DB.INPUT)                                   ## parse parameter ranges line
ymin=$(echo "${ln[@]:16:5}")                                               ## requested minimum Y height
ymax=$(echo "${ln[@]:24:5}")                                               ## requested maximum Y height
ncore=$(cat $homedir/rundb_1line_slurm.sh | grep -m1 ntasks= | cut -c 18-) ## the number of sbatch tasks AND the number of threads to allocate; takes value from sbatch --ntasks
