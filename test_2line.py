...
# # Test the two-line CLEDB inversion on synthetic or observational data
...
#     sobsa   = np.moveaxis(np.load('./obsstokes_cryonirsp_BZJOM_int1074.npy'),0,2)    ## 1074 fit data rearanged to a standard CLEDB frame [nx,ny,nw]
#     sobsb   = np.moveaxis(np.load('./obsstokes_cryonirsp_BXEMJ_int1079.npy'),0,2)    ## 1079 fit data reshaped to a standard CLEDB frame [nx,ny,nw]
#     sobs_in=[sobsa,sobsb[:,:-1,:]]   ## The Cryo-NIRSP data is a x*y*wl*4 dimensional array with one lines included.  ## create a 2line standard input for an actual observation. 
#     ## Sometimes Cryo-NIRSP datasets do not default to the same spatial scales bot both lines. One pixel is dropped from the 1079 array in this case.
...
# else:
#     print("Control parameter not set to integrated. Data will not be read properly downstream!")
...
importlib.reload(prepinv)       ## If module is modified, reload the contents
db_enc, database, dbhdr = prepinv.sdb_preprocess(yobs,dobs,keyvals,wlarr,params)  ## Warning, this might be memory consuming; see details in documentation
...
instrument = head_in[0][0]['INSTRUME'].strip()
...
fig, plots = plt.subplots(nrows=4, ncols=3, figsize=(12,8))
...
plots[1,0].set_title('Line Ratio Plasma density')
...
plt.suptitle(f"Plotting CLEDB solution no: {soln} for line combination {keyvals[4][0]}-{keyvals[4][1]} ")
plt.tight_layout()
...
## Pixel  location to be printed
xx=23       ## x pixel position
...
pnames = ["DB Index", "chi^2", "ne density", "y (height)", "x (LOS pos.)", "|B|", "varphi", "vartheta", "Bx", "By", "Bz"] ##
col_width = 12
header = "Sol".ljust(6) + "".join(name.rjust(col_width) for name in pnames)

##   Print header and table
print(header)
print("-" * len(header))

for i, row in enumerate(invout[xx, yy, :, :]):
    line = str(i).ljust(6)
    for val in row:
