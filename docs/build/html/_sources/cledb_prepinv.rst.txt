...
``rms [xs,ys,nline*4] float array``
    Returns the root mean square (:term:`RMS`) of the total counts in each Stokes profile. The rms calculation is correspondent to the ratio between intensity in the line core and background counts (the variance). This measurement shows the quality in the signal for a particular observed voxel.
