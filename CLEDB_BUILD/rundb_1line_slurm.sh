...
## MANUAL INPUT IS NOT A GOOD APPROACH FOR HEADLESS RUNS! script defines control variables statically.
ncore=36                                      ## ncore is the number of sbatch tasks AND the number of threads to allocate
...
SCRATCH=${SLURM_SCRATCH}                      ## scratch directories for the CURC system
RCsystem='rc.int.colorado.edu'                ## setup for the colorado university system; change here for your own;
homedir=pwd                                   ## record of the CLEDB_BUILD setup directory where the script is located and run from! It should follow the directory structure of the CLEDB distribution.
storedir='/projects/$whoami/solar-coronal-inversion/CLEDB_BUILD/'   ## assuming there is a dedicated storage space via a project dir for your user; following a CLEDB dir layout
## end manual variables
...
if [ $? == 0 ]; then dbexec='db*_rclinux'; fi                              ## a 3rd executable exists for this
...
if (($yh >= $ncore)); then                                                                                 ## if generating more database heights than available threads
	...
	for e in `seq 0 $(echo "scale=0 ; $ncore - 1 " | bc)`; do                                              ## cycle through available threads
		mkdir $e;
		cd $e;
		\cp '$homedir/config/'{DB.INPUT,GRID.DAT,IONEQ} ./;                                                ## make individual run dirs and copy necessary files
...
mv -r $dbdir $storedir                                                                                     ## check id directories are accurate